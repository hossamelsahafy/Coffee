  ...
  return (
    <div className="p-6">
      <Header
        ...
        CollectionName={"Origins Of Coffee"}
        search={search}
      ...
      {docs.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">
          {docs.map((item) => {
            const editUrl = `${adminRoute}/collections/Subscripe/${item.id}`;
              ...
              >
                ...
                <div>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <h3 className="text-lg font-bold text-white transition-colors group-hover:text-[#D8A46B] break-all leading-snug">
                        {item.Email}
                      </h3>
                    </div>
                  </div>
