  ...
  return (
    <>
      <form
        ...
        className="flex justify-between w-full md:w-1/2"
      >
        <div className="flex justify-between w-full">
          ...
          <button
            type="submit"
            disabled={loading}
            className={`text-base-light py-3 px-6 bg-base-dark ${locale === "en" ? "rounded-r-2xl" : "rounded-l-2xl"} hover:bg-base-border transition-all duration-300 cursor-pointer`}
          >
            {loading ? (
      ...
      {message && (
        <p
          className={`text-sm font-semibold ${status === "success" ? "text-base-coffe" : "text-amber-300"}`}
        >
          {message}
        </p>
      )}
