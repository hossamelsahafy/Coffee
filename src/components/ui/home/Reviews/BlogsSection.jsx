...
const BlogsSection = ({ Blogs }) => {
  ...
  const filteredBlogs = Blogs.filter((blog) => blog.OriginsOfCoffee?.id === selected)
  useEffect(() => {
  ...
  return (
    <div className="max-w-7xl -mt-10 text-base-light mx-auto p-4 flex md:flex-row flex-col justify-between items-stretch">
      ...
      >
        ...
        <p className="font-semibold text-3xl text-base-coffe">{t('fresh_roasted_coffee')}</p>
        <p className="w-full md:max-w-sm text-base">{t('denver_biscuit_cafe')}</p>
