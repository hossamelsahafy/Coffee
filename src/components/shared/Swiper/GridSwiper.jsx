            ...
            {enablePagePagination
              ? paginatedProducts.map((page, index) => (
                  <SwiperSlide key={index}>
                    <div
                      className={`grid ${sideBarIsOpen ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"} gap-6 w-full items-stretch`}
                    >
                      {page.map((product) => (
                        <motion.div
                          key={product.id}
